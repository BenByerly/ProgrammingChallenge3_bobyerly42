...
int main() {
    ...
    //get npt time
    currTime = (ntohl(packet.trans_ts_sec) - NTP_TIMESTAMP_DELTA);
    //print npt time
    printf("NTP time: %s", ctime(&currTime));

    //get difference
    time_t local_time_sec = mktime(localTime);
    double time_diff = difftime(currTime, local_time_sec);
    //print diff
